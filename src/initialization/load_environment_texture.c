...
{
    environment_texture_t *new_struct = NULL;

    mini_printf("    loading %s as a new env. texture.\n", asset_path);
    new_struct = malloc(sizeof(environment_texture_t));
    if (new_struct == NULL)
        return;
    new_struct->next = data->assets.environment_textures;
    new_struct->texture = sfTexture_createFromFile(asset_path, NULL);
    new_struct->name = my_strdup(asset_name);
    data->assets.environment_textures = new_struct;
}

static void overwrite_struct(data_t *data, char *asset_path,
    environment_texture_t *asset_struct)
{
    mini_printf("    loading %s by overwriting the previous env. texture.\n",
        asset_path);
